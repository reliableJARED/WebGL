...
function createCubeTower(height,width,depth){
	...
	//make a copy
	var originalPOS = new Ammo.btVector3(randX,1,randZ);
	...
	for (var h=0;h<height;h++) {
		...
		pos.setY(originalPOS.getY+(ObjBlueprint.y*h)+pos.y())
		ObjBlueprint.x = pos.x();
		ObjBlueprint.y = pos.y();
		ObjBlueprint.z = pos.z();
	}
...
function emitWorldUpdate() {
		...
		for ( var i = 0; i < rigidBodies.length; i++ ) {
			...
			if ( ms ) {
				...
				/**********************************************************************/
				/* Consider NOT SENDING Angular or Linear velocity right  */
				//get the angularvelocity of the object
			/*	var Av = obj.getAngularVelocity();
				dataToSend[(objectCount*propsPerObj)+8] =Av.x()
				dataToSend[(objectCount*propsPerObj)+9] =Av.y()
				dataToSend[(objectCount*propsPerObj)+10] =Av.z()

				//get the linearvelocity of the object
				var Lv = obj.getLinearVelocity();
				dataToSend[(objectCount*propsPerObj)+11] =Lv.x()
				dataToSend[(objectCount*propsPerObj)+12] =Lv.y()
				dataToSend[(objectCount*propsPerObj)+13] =Lv.z()	*/
				/**********************************************************************/				
				objectCount++;
