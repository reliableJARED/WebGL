...
//	var dV = accelleration * 0.01667;
	...
	for (var g=0;g<height;g++) {
				moveOver = new THREE.Vector3(0,0,0);//used for positioning rubble pieces	
				pos = obj.position.add( obj2.position );//THREE.Vector3()
				pos.add(new THREE.Vector3(0,g,0));
			//	moveOver.add(new THREE.Vector3(0,g,0));
				//moveOver.sub(new THREE.Vector3(g,0,g));
			
	for (var t=0;t<width;t++) {
		moveOver = new THREE.Vector3(0,0,0);
		pos.add(new THREE.Vector3(0,0,t));
	//	moveOver.add(new THREE.Vector3(t,0,0));
	for(var q =0; q<depth;q++){
		
		moveOver.add(new THREE.Vector3(q,0,0));	
